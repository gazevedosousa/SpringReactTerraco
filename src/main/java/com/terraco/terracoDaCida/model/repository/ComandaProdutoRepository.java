...
public interface ComandaProdutoRepository extends JpaRepository<ComandaProduto, Long> {
    ...
    @Query("Select cp from ComandaProduto cp join cp.comanda c where CAST(c.dataCriacao as date) = :dataCriacao and cp.dataExclusao is null order by cp.dataCriacao asc")
