...
public interface ComandaProdutoRepository extends JpaRepository<ComandaProduto, Long> {
    ...
    @Query("Select cp from ComandaProduto cp where DATE(cp.dataCriacao) = :dataCriacao and cp.dataExclusao is null order by cp.dataCriacao asc")
    List<ComandaProduto> findByDataCriacaoAndDataExclusaoIsNull(LocalDate dataCriacao);
    @Query("Select cp from ComandaProduto cp where MONTH(cp.dataCriacao) = :mes and YEAR(cp.dataCriacao) = :ano and cp.dataExclusao is null order by cp.dataCriacao asc")
